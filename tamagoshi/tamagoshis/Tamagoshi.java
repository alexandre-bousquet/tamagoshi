...
public class Tamagoshi {
    ...
    public boolean parler() {
        if (this.energy > 4 && this.fun > 4) {
            System.out.println(this.getName() + " : Tout va bien !");
            return true;
        } else if (this.energy <= 4 && this.fun > 4) {
            System.out.println(this.getName() + " : Je suis affamé !");
            return false;
        } else if (this.energy > 4 && this.fun <= 4) {
            System.out.println(this.getName() + " : Je meurs d'ennui !");
            return false;
        } else {
            System.out.println(this.getName() + " : Je suis affamé et je meurs d'ennui !");
            return false;
        }
    }
    ...
    public boolean mange() {
        if (this.energy < this.maxEnergy) {
            ...
            System.out.println(this.getName() + " : Miam c'est bon !");
            return true;
        } else {
    ...
    public boolean joue() {
        if (this.fun < this.maxFun) {
            this.setEnergy(this.fun + new Random().nextInt(3) + 1);
            if (this.fun > this.maxFun) {
                this.setEnergy(this.maxFun);
            }
            System.out.println(this.getName() + " : On se marre !");
            return true;
        } else {
            System.out.println(this.getName() + " : Je n'ai pas envie de jouer !");
            return false;
        }
    }
    ...
    public boolean consommeRessources() {
        ...
        this.fun--;
        if (this.energy <= 0) {
            System.out.println(this.getName() + " : Je suis KO: Arrrggh !");
            return false;
        } else if (this.fun <= 0) {
